...
4.  **Clarifying Questions (MCQs):** Generate 0-2 NEW multiple-choice questions ONLY IF the answers provided raise significant new ambiguities or require further critical details *under Indian law*. If the picture is clear, return an empty array []. Format this for the 'clarifyingQuestions' field. Do NOT repeat previous questions.
...
Structure your entire refined response according to the defined output schema. Ensure all legal references and interpretations are specific to India and reflect the new information from the answers.
`,
});
